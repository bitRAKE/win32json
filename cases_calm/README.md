...
This directory is specifically for `generated/fasm2_calm`. The conventional
flat `generated/fasm2` projection has its own end-to-end suite under
[`cases/`](../cases/README.md).

Run all cases from the repository root:

```powershell
& .\cases_calm\build.ps1
```

Products and `llvm-readobj` reports are written to `cases_calm/out/`.
